...
echo ❌ 找不到 Electron 应用
echo 📝 请先运行: npm run electron:build
pause
exit /b 1

:STOP_SERVICE
echo ⏹️  停止 OpenClaw 服务...
for /F "tokens=3" %%i in ('sc query OpenClawGateway ^| findstr STATE') do set SERVICE_STATE=%%i
...
if "!SERVICE_STATE!"=="RUNNING" (
    ...
    REM 检查网络连接
    powershell -Command "try { $tcp = New-Object System.Net.Sockets.TcpClient; $tcp.Connect('127.0.0.1', %PORT%); $tcp.Close(); Write-Host '✅ 网络：可连接'; exit 0 } catch { Write-Host '❌ 网络：无法连接'; exit 1 }"
